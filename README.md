...
- **Photos are partial.** Four venues in the NYC scenario have genuine photographs; the rest of the seed set uses placeholder imagery, which is detected at render time and visibly labeled "Placeholder image" rather than passed off as the venue. Auto-discovered venues get no placeholder at all — a real Google Places photo or an explicit "no photo found". Google's Places Photos SKU is not enabled on the project key (the API returns 200 with the `photos` field absent), so the auto-crossfade photo tour was deliberately left unbuilt rather than shipped as code that never runs.
